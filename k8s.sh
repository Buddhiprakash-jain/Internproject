...
diff match.txt cmdout.txt &> /dev/null && kubectl create deploy myd --image=buddhi82/intern:latest;kubectl scale deploy myd --replicas=5;kubectl expose deploy myd --port=80 --type=NodePort || kubectl rollout restart deployment/myd
